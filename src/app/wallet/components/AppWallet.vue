      ...
      <v-card>
        ...
        <v-dialog v-model="dialog" max-width="690">
          <v-card>
            ...
            <v-card-text>This is your address information</v-card-text>
            <v-container grid-list-md>
              <v-layout wrap>
                <v-flex xs12>
                  <v-text-field v-model="address.isvalid" label="Valid" readonly></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field v-model="address.address" label="Address" readonly></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field v-model="address.wif" label="WIF / Priv key" readonly></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field v-model="address.ismine" label="Mine" readonly></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field v-model="address.scriptPubKey" label="Script Pub Key" readonly></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field v-model="address.segid" label="Seg ID" readonly></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field v-model="address.iswatchonly" label="Watch Only" readonly></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field v-model="address.isscript" label="Is Script" readonly></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field v-model="address.iscompressed" label="Compressed" readonly></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field v-model="address.pubkey" label="Pubkey" readonly></v-text-field>
                </v-flex>
                <v-flex xs12>
                  <v-text-field v-model="address.account" label="Account" readonly></v-text-field>
                </v-flex>
              </v-layout>
            </v-container>
            ...
            <v-card-actions>
              ...
              <v-btn color="green darken-1" flat="flat" @click="dialog = false">Thanks</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
        ...
        <div class="testarea">
          <v-dialog v-model="dialogtest" hide-overlay persistent width="300">
...
export default {
  ...
  watch: {
    dialogtest(val) {
      ...
      if (this.address.pubkey != "") {
        ...
        setTimeout(
          () => ((this.dialogtest = false), (this.dialog = true)),
          timeout
        );
      } else if (
  ...
  methods: {
    ...
    dumpprivkey: function(event) {
      wallet
        .dumpprivkey(rpc, this.address.address)
        .then(resp => {
          console.log("Incoming response");
          console.log(resp);
    ...
    validateaddress: function(event) {
      ...
      util
        .validateaddress(rpc, this.address.address)
        .then(resp => {
    ...
    getaddressesbyaccount: function(event) {
      ...
      wallet
        .getaddressesbyaccount(rpc, this.address.account)
        .then(resp => {
